  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ query: text.trim(), ruleset, history }),
      });
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[300px] max-h-[500px]">
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            {msg.role === "assistant" ? (
              <KeywordText text={msg.content} className="text-body-sm text-dark leading-relaxed whitespace-pre-line" />
            ) : (
        ...
        {loading && (
          <div className="p-3 bg-bg-card border-l-2 border-accent mr-2">
            <span className="text-meta text-dark-50 animate-pulse">Consulting rulebook...</span>
          </div>
        )}
